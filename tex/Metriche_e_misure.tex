...
\begin{document}                        %label prefix:metrics
...
\begin{exercise}[2016-10-20]
  ...
  \begin{enumerate}
    ...
    \item \label{metrics:submeasure:deltaequality} Se $A,B\in \gA$ e $A\Delta B\in \gN(\eta)$, allora $\eta(A)=\eta(A\cap B)=\eta(B)$.
    \item Se $A\in \gA,\, N\in \gN(\eta)$, allora $\eta(A)=\eta(A\cup N)=\eta(A\Delta N)=\eta(A\setminus N)$
  \end{enumerate}
\end{exercise}
\begin{proof}
  \begin{enumerate}
    \item Basta dimostrare che $\gN$ è chiuso per unioni numerabili.
    ...
    \item Le ultime due uguaglianze si ottengono subito dal \cref{metrics:submeasure:deltaequality} perché $N=(A\Delta N)\Delta(A\setminus N)=(A\cup N)\Delta(A\setminus N)$. Inoltre per monotonia $\eta(A\setminus N)\leq\eta(A)\leq\eta(A\cup N)=\eta(A\setminus N)$, da cui la prima uguaglianza.
  \end{enumerate}
\end{proof}
\begin{exercise}[2016-10-20]
